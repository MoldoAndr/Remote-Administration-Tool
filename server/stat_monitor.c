...
void write_statistics()
{
    ...
    file_size = strlen(processed_output) + 1;

    fd = open(STATS_FILE, O_CREAT | O_RDWR | O_TRUNC, 0644);
    if (fd == -1)
    {
    ...
    mapped_mem = mmap(NULL, file_size, PROT_WRITE, MAP_SHARED, fd, 0);
    if (mapped_mem == MAP_FAILED)
    {
        fprintf(stderr, "mmap failed: %s\n", strerror(errno));
        goto cleanup;
    }
    memcpy(mapped_mem, processed_output, file_size);
